    ...
    public static class Mutable {
        ...
        public boolean tryRemove(ItemStack stack, SlotAccess access, Player player) {
            if (item.hasCraftingRemainingItem()) {
                if (stack.is(item.getCraftingRemainingItem())) {
            ...
            if (count < CAPACITY) {
                if (item != Items.AIR) { //putting items in full tiffin
                    if (stack.getItem() == item) {
                        ...
                        if (i > CAPACITY) { //cannot fit all
                            ...
                            if (stack.getItem().getCraftingRemainingItem() != null) {
                                player.addItem(new ItemStack(stack.getItem().getCraftingRemainingItem(), CAPACITY - count));
                            }
                            count = CAPACITY;
                            ...
                            count = i;
                            if (stack.getItem().getCraftingRemainingItem() != null) {
                                access.set(new ItemStack(stack.getItem().getCraftingRemainingItem(), stack.getCount()));
                            } else {
                    ...
                    if (stack.getCount() > CAPACITY) { //cannot fit all of stack
                        count = CAPACITY;
                        if (stack.getItem().getCraftingRemainingItem() != null) {
                            access.set(new ItemStack(stack.getItem().getCraftingRemainingItem(), stack.getCount() - CAPACITY));
                            player.addItem(new ItemStack(stack.getItem().getCraftingRemainingItem(), CAPACITY));
                        } else {
                            access.set(new ItemStack(Items.AIR));
                        }
                        return CAPACITY;
