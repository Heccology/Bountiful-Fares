...
public record TrellisPlantDefinition(Item plant, ResourceLocation model) {
    public static final Codec<TrellisPlantDefinition> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            ResourceLocation.CODEC.fieldOf("model").forGetter(TrellisPlantDefinition::model)
    ).apply(instance, TrellisPlantDefinition::new));

    public TrellisPlantDefinition(Item plant, ResourceLocation model) {
        this.plant = plant;
        this.model = model;
    }
}
