...
public class HangingLemonBlock extends HangingFruitBlock {
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        return Block.sideCoversSmallSquare(world, pos.up(), Direction.DOWN) && !world.isWater(pos)
                || world.getBlockState(pos.up()).isOf(BFBlocks.LEMON_LEAVES) && !world.isWater(pos)
                || world.getBlockState(pos.up()).isOf(BFBlocks.FLOWERING_LEMON_LEAVES) && !world.isWater(pos);
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (i == 4) {
            HangingFruitBlock.dropStack(world, pos, new ItemStack(BFItems.LEMON, 1));
