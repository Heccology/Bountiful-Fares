...
public class OrangeBlock extends FruitBlock{
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (player.canConsume(false)) {
            world.setBlockState(pos, ModBlocks.EATEN_ORANGE_BLOCK.getDefaultState().with(EatenFruitBlock.FACING, direction), Block.NOTIFY_LISTENERS);
    ...
    public void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        if (!world.isClient) {
            ...
            world.spawnEntity(new ItemEntity(world, hit.getBlockPos().getX() + 0.5, hit.getBlockPos().getY() + 0.5, hit.getBlockPos().getZ() + 0.5, new ItemStack(ModItems.ORANGE, 9)));
