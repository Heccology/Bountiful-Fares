...
public class CeramicTileSlabBlock extends SlabBlock implements DyeableCeramicBlockInterface, BlockEntityProvider {
    ...
    public CeramicTileSlabBlock(Settings settings) {
        super(settings);
        this.setDefaultState(this.getDefaultState().with(TYPE, SlabType.BOTTOM).with(WATERLOGGED, false).with(CHECKERED, false).with(WAXED, false));
    }
    @Override
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if ((itemStack.isOf(Items.SPONGE) || itemStack.isOf(Items.WET_SPONGE)) && !checkered && !waxed && CeramicTilesBlockEntity.getColor(world, pos) != CeramicTilesBlockEntity.DEFAULT_COLOR) {
            world.setBlockState(pos, ModBlocks.CERAMIC_TILE_SLAB.getStateWithProperties(state).with(CHECKERED, true), 2);
            ...
            world.setBlockState(pos, ModBlocks.CERAMIC_TILE_SLAB.getStateWithProperties(state).with(WAXED, true), 2);
            ...
            world.setBlockState(pos, ModBlocks.CERAMIC_TILE_SLAB.getStateWithProperties(state).with(WAXED, false), 2);
            world.playSound(player, player.getX(), player.getY(), player.getZ(), SoundEvents.BLOCK_BIG_DRIPLEAF_TILT_UP, SoundCategory.BLOCKS, 1.0F, 1.0F);
            return ActionResult.SUCCESS;
            ...
            world.setBlockState(pos, ModBlocks.CERAMIC_TILE_SLAB.getDefaultState().with(TYPE, state.get(TYPE)).with(WATERLOGGED, state.get(WATERLOGGED)), 2);
            world.playSound(player, player.getX(), player.getY(), player.getZ(), SoundEvents.BLOCK_BIG_DRIPLEAF_TILT_UP, SoundCategory.BLOCKS, 1.0F, 1.0F);
            return ActionResult.SUCCESS;
        }
